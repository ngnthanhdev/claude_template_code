...
`references/principles.md`. That skill owns the *taste* call (when/why);
...
- **Library list** (installed once at the foundation layer):
  ...
  - `@shopify/flash-list` — virtualized lists (see `mobile-patterns` for
...
Check `references/principles.md` first — this recipe communicates scroll
...
`mobile-patterns`'s `FlashList` feed and expect it to behave the same.

## Recipe: Swipe-to-island morph

Check `references/principles.md` first — this recipe is a good fit for
...
Check `references/principles.md` first — direct-manipulation gestures
...
Check `references/principles.md` first — a carousel is usually the "main
...
list in `mobile-patterns`.

## Recipe: Skia effects (particles / shaders / blur)

Check `references/principles.md` first — reserve GPU-canvas effects for
...
- Check `references/principles.md` before adding any recipe here — this
