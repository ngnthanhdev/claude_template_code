...
`references/threat-model.md` establishes for the mobile↔API boundary. Concretely:
...
- **Every authorization decision the mobile app appears to make must be
  independently re-checked by the NestJS API** (`references/backend.md`'s
...
`mobile-patterns`'s implementation for the exact pattern this template
...
Same discipline as `references/review.md`, MASVS in place of ASVS:
