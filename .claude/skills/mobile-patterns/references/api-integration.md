...
- The 401 → refresh → retry-once flow here calls into `references/auth-state.md`'s
...
- `networkMode: "offlineFirst"` means queries resolve from cache
  ...
  with `references/app-composition.md`'s `useIsOnline()` banner for user-visible offline
  indication.
...
- Invalidate the narrowest key that covers everything the mutation could
  ...
  cache.
- See `references/data-forms.md` for the optimistic-update variant of this same
...
- Don't reimplement token refresh per call site — reuse
  `references/auth-state.md`'s deduped refresh.
