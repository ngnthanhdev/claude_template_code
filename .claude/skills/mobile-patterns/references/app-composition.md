...
- One screen = one route file. Don't put business logic or data fetching
  ...
  (`references/api-integration.md`) so the screen stays a thin composition layer.
...
- Use dynamic segments (`app/(app)/post/[id].tsx`) for detail screens rather
  ...
  or read from the query cache (`references/api-integration.md`) on the detail
  screen.
...
- Combine with TanStack Query's cache (`references/api-integration.md`): render
...
- Queue-and-retry for writes is `references/api-integration.md` + `references/data-forms.md`'s
...
- **Animations:** anything that must track a gesture (scroll, pan, drag)
  ...
  manual `setState` per frame — see `animations`.
...
- Don't fetch data directly inside a screen component body — that's
  `references/api-integration.md`'s hook layer.
...
- Don't do per-frame work (gesture tracking, scroll-driven transforms) in
  JS-thread state — see `animations` for the UI-thread pattern.
