...
`references/nestjs.md` decides **how Nest produces it**.
...
URI versioning, enabled once in `main.ts` (see `references/nestjs.md`), so every
...
- **Cursor pagination, not offset.** `cursor` is the last-seen row's `id`;
  ...
  `references/prisma.md` for the Prisma query that fulfills this contract.
...
`message` to decide behavior — that's what `code` is for. `references/nestjs.md`'s
...
| `401 Unauthorized` | Missing/invalid/expired credentials — see `security` |
| `403 Forbidden` | Valid credentials, insufficient permission (RBAC) — see `security` |
...
- Don't let an endpoint hand-roll its own error shape — that's
  `references/nestjs.md`'s global exception filter's job, applied once, globally.
