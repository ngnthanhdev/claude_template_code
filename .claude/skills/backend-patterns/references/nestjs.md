...
schemas, one global exception filter producing `references/api-design.md`'s error
...
the exception filter below and turned into `references/api-design.md`'s `422` envelope, so
...
Guards decide **auth/RBAC** (`security`'s `JwtAuthGuard`/
...
boot (see `security` for how this backs the JWT strategy).

## Global exception filter

The single place that turns *any* thrown error — a Nest `HttpException`, a
`ZodValidationException`, or a truly unexpected bug — into `references/api-design.md`'s
error envelope:
...
- Don't put authorization logic inside an interceptor or logging inside a
  guard — see `security` for where guards belong.
