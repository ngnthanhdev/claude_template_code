...
should start until every task below is checked and its acceptance criteria
are met.

Each task follows the same block format: a checkbox, the files it touches,
its acceptance criteria, and (where relevant) which skills to load. Tasks in
this layer are independent enough to fan out via `/run-layer`, except where a
task explicitly notes a same-layer dependency.

---

### [ ] Task: Scaffold the Expo app in `apps/mobile`

**Files:** `apps/mobile/**` (new Expo project), `apps/mobile/app.json`,
...
### [ ] Task: Install the animation dependency stack
...
- `react-native-reanimated@4`, `react-native-gesture-handler`,
  ...
  `react-native-reanimated-carousel` are installed at versions compatible with
  the Expo SDK selected for this project.
...
- Documented in this task's summary: the exact Expo SDK version chosen and
  confirmation it supports `newArchEnabled: true` with these package versions
  (per the compatibility risk noted in the design spec, §13).

**Depends on:** the Expo scaffold task above (same layer, run after it).

---

### [ ] Task: Scaffold the NestJS API in `apps/api`

**Files:** `apps/api/**` (new NestJS project), `apps/api/package.json`,
`apps/api/tsconfig.json`, `apps/api/prisma/schema.prisma`
...
**Files:** `packages/shared/**`, `packages/shared/package.json`,
`packages/shared/tsconfig.json`, `packages/shared/src/index.ts`
...
### [ ] Task: Wire CI against the scaffolded skeleton

**Files:** none new — verifies `.github/workflows/ci.yml` (already shipped by
the template) against the now-populated `apps/*` and `packages/shared`.
...
- Any turbo pipeline gaps discovered (e.g. a package missing a `lint` or
  `test` script that `turbo.json` expects) are fixed so the root scripts work
  uniformly across `apps/mobile`, `apps/api`, and `packages/shared`.

**Depends on:** all four tasks above.

---

## After this layer: creating `layer-1-todo.md`

Once every task above is checked and `pnpm turbo run lint typecheck test`
